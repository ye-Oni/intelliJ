...
public class UserDao {
    ...
    public int createUser(PostUserReq postUserReq) {
        String createUserQuery = "insert into User (email, password, name) VALUES (?,?,?)"; // 실행될 동적 쿼리문
        Object[] createUserParams = new Object[]{postUserReq.getEmail(), postUserReq.getPassword(), postUserReq.getName()}; // 동적 쿼리의 ?부분에 주입될 값
        this.jdbcTemplate.update(createUserQuery, createUserParams);
        // email -> postUserReq.getEmail(), password -> postUserReq.getPassword(), name -> postUserReq.getName() 로 매핑(대응)시킨다음 쿼리문을 실행한다.
        // 즉 DB의 User Table에 (email, password, name)값을 가지는 유저 데이터를 삽입(생성)한다.
    ...
    public int checkName(String name) {
        String checkNameQuery = "select exists(select name from User where name = ?)"; // User Table에 해당 name을 가진 유저 정보가 존재하는가?
        String checkNameParams = name;  // 확인할 이름
        return this.jdbcTemplate.queryForObject(checkNameQuery, int.class, checkNameParams);
    }

    // 로그인된 사용자의 이메일 존재 여부 확인
    public int loginUser(PostLoginReq postLoginReq) {
        String loginUserQuery = "insert into User (email, password) VALUES (?,?)"; // 실행될 동적 쿼리문
        Object[] loginUserParams = new Object[]{postLoginReq.getEmail(), postLoginReq.getPassword()}; // 동적 쿼리의 ?부분에 주입될 값
    ...
    public int modifyUserName(PatchUserReq patchUserReq) {
        String modifyUserNameQuery = "update User set name = ? where userID = ? "; // 해당 userID를 만족하는 User를 해당 name으로 변경한다.
        Object[] modifyUserNameParams = new Object[]{patchUserReq.getName(), patchUserReq.getUserID()}; // 주입될 값들(name, userID) 순
    ...
    public User getPwd(PostLoginReq postLoginReq) {
        String getPwdQuery = "select userID, password,email,name from User where email = ?"; // 해당 email을 만족하는 User의 정보들을 조회한다.
