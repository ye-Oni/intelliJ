...
public class UserController {
    ...
    public BaseResponse<PostUserRes> createUser(@RequestBody PostUserReq postUserReq) {
        ...
        // password에 값이 존재하는지, 빈 값으로 요청하지는 않았는지 검사하기.
        if (postUserReq.getPassword() == null) {
            return new BaseResponse<>(POST_USERS_EMPTY_PWD);
        }
        // 비밀번호 정규 표현 : 최소 8자리에 숫자, 대문자, 소문자, 특수문자 각각 1개 이상 포함.
        if (!isRegexPassword(postUserReq.getPassword())) {
            return new BaseResponse<>(POST_USERS_INVALID_PASSWORD);
        }
        // name에 값이 존재하는지, 빈 값으로 요청하지는 않았는지 검사하기.
    ...
    public BaseResponse<List<GetUserRes>> getUsers(@RequestParam(required = false) String name) {
        ...
        try {
            if (name == null) { // query string인 name이 없을 경우, 그냥 전체 유저정보를 불러온다.
