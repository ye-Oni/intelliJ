...
public class UserService {
    ...
    public PostUserRes createUser(PostUserReq postUserReq) throws BaseException {
        ...
        // 중복 확인: 해당 이름/닉네임을 가진 유저가 있는지 확인합니다. 중복될 경우, 에러 메시지를 보냅니다.
...
//  *********************************************************************
    ...
    public PostLoginRes loginUser(PostLoginReq postLoginReq) throws BaseException {
            // 이메일(회원) 존재 여부 확인: 해당 이메일을 가진 유저가 있는지 확인한다. 없을 경우, 에러 메세지를 보냅니다.
            if (userProvider.checkEmail(postLoginReq.getEmail()) == 0) {
                throw new BaseException(FAILED_TO_LOGIN);
        }
            try {
                int userID = userDao.loginUser(postLoginReq);
